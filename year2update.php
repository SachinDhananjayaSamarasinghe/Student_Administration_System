...
// update data
        if ($valid) {
            ...
            $sql = "UPDATE year2  set sub1 = ?, sub2 = ?, sub3 = ?,  sub4 = ?, sub5 = ?, sub6 =?, sub7 = ?, sub8 = ?, sub9 = ?, sub10 = ?, sub11 =?, sub12 = ?, sub13 = ?, sub14 =?, sub15 = ?, sub16 = ?, email =? WHERE id = ?";
            $q = $pdo->prepare($sql);
            $q->execute(array($mi,$mii,$bio,$cs,$es,$chem, $phy, $il, $cmii, $itii, $ei, $bc, $itt, $we, $sh, $eii, $email,$id));
            Database::disconnect();
        ...
        $pdo = Database::connect();
        $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $sql = "SELECT * FROM year2 where id = ?";
        $q = $pdo->prepare($sql);
        $q->execute(array($id));
...
<body>
    <div class="container">
            <div class="row">
             <legend><h3>Year 2 Courses - Admin Update</h3></legend>   
            </div>
     ...
     <br><br>
            ...
            <br>
    <form action="year2update.php?id=<?php echo $id?>" method="post">
...
<br><br>
        ...
        <a href="year2info.php" class="btn">&nbsp &nbsp Back &nbsp &nbsp</a>
<br><br>       
